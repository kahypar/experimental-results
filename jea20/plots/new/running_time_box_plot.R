...
running_time_per_pin_box_plot <- function(dataframes, 
  ...
  running_time = ggplot(result, aes(x=algorithm, y=avg_time_per_pin)) +
    ...
    geom_text(aes(x = algorithm, y = 0.5 * max_pow10_time, label=round(gmean_time, 1), group = algorithm), result_gmean, 
              size = plot_text_size(T),  position = position_dodge(width=0.75)) +
    ...
    create_theme(latex_export, T, small_size, legend_position = "none", x_axis_text_angle = text_angle, x_axis_text_hjust = 1)
  if ( contains_invalid_results ) {

...
# Read Data Frames
instances <- read.csv("km1/instances.csv", header = TRUE)
kahypar_k <- aggreg_data(read.csv("km1/km1_kKaHyPar.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
patoh_d <- aggreg_data(read.csv("km1/km1_patoh_d.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
patoh_q <- aggreg_data(read.csv("km1/km1_patoh_q.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
hmetis_r <- aggreg_data(read.csv("km1/km1_hmetis-r.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
hmetis_k <- aggreg_data(read.csv("km1/km1_hmetis-k.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
zoltan_alg_d <- aggreg_data(read.csv("km1/km1_zoltan-algd.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
mondriaan <- aggreg_data(read.csv("km1/km1_mondriaan.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
hype <- aggreg_data(read.csv("km1/km1_hype.csv", header = TRUE), timelimit = 28800, epsilon = 0.03)
...
pdf(file = "img/figure_10.pdf", width = 3 * 2.1666, height = 2 * 1.666666) 
print(p1)
dev.off()
...
p2 <- running_time_per_pin_box_plot(list(kahypar_k,
                                         patoh_d,
                                         patoh_q,
                                         hmetis_r,
                                         hmetis_k,
...
p3 <- performace_plot(list(kahypar_k,
                           patoh_d,
                           patoh_q,
                           hmetis_r,
                           hmetis_k,
...
performance_plot <- egg::ggarrange(p3,p2, widths = c(1,1), ncol = 2,  draw=F)
pdf(file = "img/figure_9.pdf", width = 3 * 2.1666, height = 1.5 * 1.666666) 
